...
class rm_categ_report_parser(report_sxw.rml_parse):
	def __init__(self, cr, uid, name, context=None):
		if context is None:
			context = {}
		super(rm_categ_report_parser, self).__init__(cr, uid, name, context=context)
		
		self.localcontext.update({
			'time': time,
		})

	def _get_mutation(self, rm_categ_ids=False, context=None):
		"""
		...
		@return:
		"""
		product_obj = self.pool.get('product.rm.category')
		if context is None:
			context = {}
		context.update({
		...
		return product_obj.get_product_available(self.cr, self.uid, rm_categ_ids, context=context)

	def set_context(self, objects, data, ids, report_type=None):
		rm_categ_obj = self.pool.get('product.rm.category')
		rm_categ_ids = rm_categ_obj.search(self.cr, self.uid, [])
		objects = rm_categ_obj.browse(self.cr, self.uid, rm_categ_ids)

		states = ['done']
		product_opening = self._get_mutation(rm_categ_ids, {'from_date':data['from_date'],'to_date':data['to_date'],\
								'what':['all_in', 'all_out'],'states':states})
		product_incoming = self._get_mutation(rm_categ_ids, {'from_date':data['from_date'],'to_date':data['to_date'],\
								'what':['in'],'states':states})
		product_outgoing = self._get_mutation(rm_categ_ids, {'from_date':data['from_date'],'to_date':data['to_date'],\
								'what':['out'],'states':states})
		product_adjustment = self._get_mutation(rm_categ_ids, {'from_date':data['from_date'],'to_date':data['to_date'],\
								'what':['adj_in','adj_out'],'states':states})
		product_closing = self._get_mutation(rm_categ_ids, {'from_date':data['from_date'],'to_date':data['to_date'],\
								'what':['all_in','all_out','in','out2','adj_in','adj_out'],'states':states})

		prod_product_opening = self._get_mutation(rm_categ_ids, {'from_date':data['from_date'],'to_date':data['to_date'],\
								'what':['prod_all_IN', 'prod_all_OUT'],'states':states})
		prod_product_incoming = self._get_mutation(rm_categ_ids, {'from_date':data['from_date'],'to_date':data['to_date'],\
								'what':['prod_IN'],'states':states})
		prod_product_outgoing = self._get_mutation(rm_categ_ids, {'from_date':data['from_date'],'to_date':data['to_date'],\
								'what':['prod_OUT'],'states':states})
		prod_product_waste = self._get_mutation(rm_categ_ids, {'from_date':data['from_date'],'to_date':data['to_date'],\
								'what':['prod_waste'],'states':states})
		prod_product_adjustment = self._get_mutation(rm_categ_ids, {'from_date':data['from_date'],'to_date':data['to_date'],\
								'what':['prod_adj_IN3','prod_adj_OUT3'],'states':states})
		prod_product_closing = self._get_mutation(rm_categ_ids, {'from_date':data['from_date'],'to_date':data['to_date'],\
		...
		return super(rm_categ_report_parser, self).set_context(
			objects, data, ids, report_type=report_type)
# report_sxw.report_sxw('report.beacukai.out.form':'beacukai.document.line':'beacukai/report/beacukai_line.mako', parser=rm_categ_report_parser, header=False)
...
class rm_categ_report_xls(report_xls):
	...
	def generate_xls_report(self, parser, xls_style, data, objects, wb):
		...
		report_name = 'Laporan Pertanggungjawaban Bahan Baku'
		c_specs = [
...
rm_categ_report_xls('report.rm.categ.mutation.report.xls','product.rm.category', parser=rm_categ_report_parser, header=False)

class rm_categ_wip_report_xls(report_xls):
...
rm_categ_wip_report_xls('report.rm.categ.wip.report.xls','product.rm.category', parser=rm_categ_report_parser, header=False)