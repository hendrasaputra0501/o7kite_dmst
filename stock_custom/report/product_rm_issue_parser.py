...
class product_rm_issue_parser(report_sxw.rml_parse):
	def __init__(self, cr, uid, name, context=None):
		if context is None:
			context = {}
		super(product_rm_issue_parser, self).__init__(cr, uid, name, context=context)
		
		self.localcontext.update({
			'time': time,
			'report_title' : 'Laporan Pemakaian Bahan Baku'
			})
	...
	def set_context(self, objects, data, ids, report_type=None):
		...
		move_line_ids = self.pool.get(brw_model).search(self.cr, self.uid, [
								...
								('product_id.product_type','=','raw_material'),
								])
		...
		return super(product_rm_issue_parser, self).set_context(
...
_column_sizes = [
	...
	('product_qty', 15),
	('product_qty_sub', 15),
	('subcont_supp_name', 30),
]

class product_rm_issue_xls(report_xls):
	...
	def generate_xls_report(self, parser, xls_style, data, objects, wb):
		...
		ws.write_merge(row_pos, row_pos, 1, 2, "Bukti Pengeluaran", cell_style_center)
		...
		ws.write(row_pos+1, 6, "Digunakan", cell_style_center)
		ws.write(row_pos+1, 7, "Disubkontrakkan", cell_style_center)
		ws.write_merge(row_pos, row_pos+1, 8, 8, "Penerima\nSubkontrak", cell_style_center)
		row_pos+=2
		...
		for line in objects:
			...
			c_specs = [
					...
					('issue_no', 1, 0, 'text', line.picking_id and line.picking_id.name or '', None, ll_cell_style),
				]
			
			try:
				c_specs += [
					('issue_date', 1, 0, 'date', datetime.strptime(
						line.date, '%Y-%m-%d %H:%M:%S'), None,
					 ll_cell_style_date),
				]
			except:
				c_specs += [
					('issue_date', 1, 0, 'text', None),
				]
...
product_rm_issue_xls('report.stock.rm.issue.xls','stock.move', parser=product_rm_issue_parser, header=False)